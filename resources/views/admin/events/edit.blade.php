    ...
    <x-slot name="header">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-xs sm:text-sm text-gray-600 mt-1">Update event information</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto">
        <form action="{{ route('admin.events.update', $event) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="dojo_id" id="dojo_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 @error('dojo_id') border-red-500 @enderror">
                        ...
                        @foreach($dojos as $dojo)
                            <option value="{{ $dojo->id }}" data-address="{{ $dojo->address }}" {{ old('dojo_id', $event->dojo_id) == $dojo->id ? 'selected' : '' }}>{{ $dojo->name }}</option>
                        @endforeach
                    </select>
                    @error('dojo_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 @error('description') border-red-500 @enderror">{{ old('description', $event->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="event_date" id="event_date" value="{{ old('event_date', \Carbon\Carbon::parse($event->event_date)->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="registration_deadline" id="registration_deadline" value="{{ old('registration_deadline', $event->registration_deadline ? \Carbon\Carbon::parse($event->registration_deadline)->format('Y-m-d') : '') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="registration_fee" id="registration_fee" value="{{ old('registration_fee', $event->registration_fee ?? 0) }}" min="0" step="0.01"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            dojoSelect.addEventListener('change', function() {
                ...
                // Only fill if there's an address
