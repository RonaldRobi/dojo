<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <form method="GET" action="{{ route('admin.reports.revenue') }}" class="space-y-4">
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.reports.revenue') }}" class="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium">Reset</a>
                    
                    <!-- Export Buttons -->
                    <div class="ml-auto flex gap-2">
                        <a href="{{ route('admin.reports.revenue', array_merge(request()->all(), ['export' => 'csv'])) }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                            </svg>
                            CSV
                        </a>
                        <a href="{{ route('admin.reports.revenue', array_merge(request()->all(), ['export' => 'pdf'])) }}" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold mt-2">{{ number_format($totalInvoices) }}</h3>
                    </div>
                    <div class="bg-white bg-opacity-20 rounded-full p-3">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-purple-100 text-sm font-medium">Average per Invoice</p>
                        <h3 class="text-3xl font-bold mt-2">RM {{ $totalInvoices > 0 ? number_format($totalRevenue / $totalInvoices, 0) : 0 }}</h3>
                    </div>
                    <div class="bg-white bg-opacity-20 rounded-full p-3">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h3 class="text-lg font-semibold text-gray-900">Revenue by Dojo</h3>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-700 uppercase">Total Revenue</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($revenueByDojo as $item)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $item['dojo'] }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $item['count'] }}</td>
                                <td class="px-6 py-4 text-sm text-right font-semibold text-gray-900">RM {{ number_format($item['total'], 0) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($invoices as $invoice)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $invoice->member->dojo->name ?? 'N/A' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $invoice->member->fullname ?? 'N/A' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ ucfirst(str_replace('_', ' ', $invoice->invoice_type)) }}</td>
                                <td class="px-6 py-4 text-sm text-right font-semibold text-gray-900">RM {{ number_format($invoice->total_amount, 0) }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-6 py-8 text-center text-gray-500">No revenue data for this period</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
