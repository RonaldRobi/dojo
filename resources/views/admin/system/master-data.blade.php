<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage system master data</p>
        </div>
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-200 bg-gray-50">
            <form method="GET" class="flex gap-4">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search..." class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
                ...
                <a href="{{ route('admin.settings.master-data') }}" class="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">Reset</a>
            </form>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($masterData as $data)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ucfirst($data->type) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $data->code ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $data->dojo->name ?? 'Global' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $data->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
        ...
        <div class="px-6 py-4 border-t border-gray-200">{{ $masterData->links() }}</div>
    </div>
</x-app-layout>
